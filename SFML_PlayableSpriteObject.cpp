	...
	, m_moveSpeed(200)
{
	...
	m_bulletTexture2.loadFromFile("Media/Textures/bullet2.png");
}
...
SFML_BulletSpriteObject* SFML_PlayableSpriteObject::processEvents(float elapsedTime, sf::Transform transform, sf::Vector2f mouseScreenPosition)
{
	...
	else if (m_equippedWeapon = PISTOL)
	{
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::R))
		{
			ReloadPistol();
...
void SFML_PlayableSpriteObject::ReloadPistol()
{
	m_playerAction = IDLE;
	setCurrentAnimation(m_pistolReloadAnimationID);
	m_equippedWeapon = PISTOL;
...
void SFML_PlayableSpriteObject::ReloadRifle()
{
	m_playerAction = IDLE;
...
SFML_BulletSpriteObject* SFML_PlayableSpriteObject::shoot()
{
	...
	if (toShootState())
	{
			...
			bullet->applyTexture(&m_bulletTexture2);

			if (m_equippedWeapon == RIFLE)
			{
				bullet->setPosition(getTransform().transformPoint(296 - 98, 156 - 124));
			}
		
		
			else if (m_equippedWeapon == PISTOL)
			{
				bullet->setPosition(getTransform().transformPoint(249 - 104, 167 - 125));
			}
